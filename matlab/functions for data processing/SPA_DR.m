function [data_SPA, var_sel,var_sel_phase2] = SPA_DR(data,labels,ratio,m_min,m_max,autoscaling,wavelengths)
    % input：data-Spectral Data Matrix
    %        labels-Category tags for visualization
    %        ratio-Model Division Scale
    %        m_min&m_max-Limitation of the band interval for extraction
    %        autoscaling-whether or not to use autoscaling(0 or 1)
    %        wavelengths-Input Bands
    % output：Result after dimensionality reduction

[trainIdx, testIdx] = dividerand(length(data), ratio, 1-ratio);
Xcal = data(trainIdx,:);Xval = data(testIdx,:);
ycal = labels(trainIdx,:);yval = labels(testIdx,:);
...
N = size(Xcal,1); % Number of calibration samples
...
plot(RMSEP_scree)
xlabel('Number of bands included in the model'),ylabel('RMSE')
...
% title(['Final number of selected bands: ' num2str(length(var_sel)) '  (RMSE = ' num2str(RMSEP_scree(i_crit)) ')'])
...
% Presents the selected variables in the first object of the calibration set
figure;
plot(wavelengths, data(1,:), "LineWidth", 1, "Color","blue");hold on;
plot(wavelengths(var_sel),data(1,var_sel), "ro")
grid off;
legend('Spectra','Selected bands')
xlabel('Wavelength/nm')
ylabel('Reflectance');


data_SPA = data(:,var_sel);
